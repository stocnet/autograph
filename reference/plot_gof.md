...
[`RSiena::sienaGOF()`](https://rdrr.io/pkg/RSiena/man/test_gof.html),
`MoNAn::monanGOF()`, or the 'ergm' package's gof() function. Internally,
the GOF object is translated into a common class (`ag_gof`), which has
its own plot method to ensure a consistent look and feel. It is not
expected that users will create `ag_gof` class objects themselves.
...
- cumulative:
  ...
  `sienaGOF()` for `{RSiena}`, but treated within the plotting function
  for `{MoNAn}` and 'ergm'.
...
The function uses the highlight colour defined in the current theme to
highlight the observed statistics. The function also uses the base
colour defined in the current theme to draw the violin and box plots.
...
the fact using `{ggplot2}` functions such as
[`ggtitle()`](https://ggplot2.tidyverse.org/reference/labs.html) or
[`labs()`](https://ggplot2.tidyverse.org/reference/labs.html).

The user can choose whether to plot the statistics cumulatively or not.
This is typically handled within
[`RSiena::sienaGOF()`](https://rdrr.io/pkg/RSiena/man/test_gof.html),
but for `MoNAn::monanGOF()` and the 'ergm' package's gof() function the
...
Other MoNAn:
[`plot_convergence`](https://stocnet.github.io/autograph/reference/plot_convergence.md)

Other RSiena:
[`plot_interp`](https://stocnet.github.io/autograph/reference/plot_interp.md)
