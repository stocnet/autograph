...
#
# Ported from {manynet}'s data-raw/build_tutorial_articles.R (via {netrics})
# so that the stocnet packages share one article-generation transform. Only
# the list of build_article() calls at the bottom differs. It replaces the
# earlier one-off data-raw/tute2article.py, which hard-coded absolute paths
# and so could not be run in CI.
#
...
build_article(
  "inst/tutorials/autograph1/visualisation.Rmd",
  "vignettes/articles/visualising-networks.Rmd"
)

# Add a build_article() call here for each further tutorial as it is written.
