...
#' @param method How the levels should be laid out for "multilevel" layout:
...
#' @param alpha,beta The angles, in degrees, at which the levels of a
#'   "multilevel" layout are projected onto the plane.
...
#' @export
layout_multilevel <- function(.data, level,
...
# Levels for `layout_multilevel()` when none were given. A network already
...
.check_level_reach <- function(.data) {
  ...
  manynet::snet_abort(
    "The {.val multilevel} layout places the levels by the distances between",
...
.drop_unusable_weights <- function(.data) {
  ...
  manynet::snet_info(
    "Ignoring the tie weights, because the {.val multilevel} layout places",
...
.check_level_ties <- function(.data, level, method) {
  ...
  manynet::snet_abort(
    "The {.val {method}} method for the {.val multilevel} layout lays out",
