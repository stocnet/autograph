...
Network visualisation is non-trivial; indeed very important.
...
Drawing network graphs is also a crucial way to explore and understand
both the global structure of a network as well as 
smaller-scale structures such as nodal positions or communities within it.
...
`{autograph}` builds upon `{ggraph}` for quickly and informatively graphing networks,
and uses `{ggplot2}` for plotting related statistics and other information.
It also depends on the coercion routines available in `{manynet}` to make it easy to use 
the functions included in this package with many related network analysis packages.
...
the `graphr()` function in `{autograph}` is a quick and easy way to obtain a 
clear first look of the network for preliminary investigations and understanding of the network. 
Let's quickly visualise one of the `ison_` datasets included in the `{manynet}` package.
